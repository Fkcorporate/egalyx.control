...
# ============================================================
# MOTEUR DE COTATION ISO 31000 / 31010
# ============================================================

TAUX_REDUCTION_MAITRISE = {
    1: 0.00,   # Très faible  : aucun contrôle
    2: 0.10,   # Faible       : contrôles informels
    3: 0.25,   # Moyenne      : contrôles documentés
    4: 0.40,   # Bonne        : contrôles testés
    5: 0.60,   # Très bonne   : contrôles continus et audités
}


def _qualifier_score(score):
    """Qualifie un score numérique en (niveau, couleur)."""
    if score is None:
...
def get_taux_reduction(niveau_maitrise):
    """Retourne le taux de réduction de probabilité selon le niveau de maîtrise."""
    if not niveau_maitrise:
        return 0.0
    try:
        return TAUX_REDUCTION_MAITRISE.get(int(niveau_maitrise), 0.0)
    except (ValueError, TypeError):
        return 0.0


def calculer_maitrise_depuis_dmr(dispositifs):
    """
    Calcule un niveau de maîtrise suggéré (1-5) à partir des DMR du risque.
    Formule ISO 31010 : 60 % efficacité + 40 % couverture, pondérée par type.
    Retourne un float (ex: 3.0) ou None si aucun dispositif évalué.
    """
    if not dispositifs:
        return None
    ...
    for dmr in dispositifs:
        if getattr(dmr, 'is_archived', False):
            continue

        efficacite = dmr.efficacite_reelle or 0
        couverture = dmr.couverture or 0

        if efficacite == 0:
            continue

        score_dmr = (efficacite * 0.6) + (couverture * 0.4)
        poids = poids_type.get(getattr(dmr, 'type_dispositif', None), 0.7)

        total_score += score_dmr * poids
        total_poids += poids
...
def verifier_coherence_maitrise(niveau_saisi, niveau_dmr):
    """
    Retourne (statut, message) selon l'écart entre jugement et DMR.
    statut ∈ {'success', 'warning', 'danger', 'info'}
    """
    if niveau_dmr is None:
        return ('info',
                "Aucun dispositif de maîtrise évalué — jugement basé sur votre expertise seule.")

    if not niveau_saisi:
        return ('info', f"Niveau suggéré par les DMR : {niveau_dmr}/5")

    ecart = abs(float(niveau_saisi) - float(niveau_dmr))

    if ecart <= 0.5:
        return ('success',
                f"✅ Cohérent avec vos dispositifs de maîtrise (écart : {ecart:.1f})")
    elif ecart <= 1.5:
        return ('warning',
                f"⚠️ Léger écart ({ecart:.1f}) entre votre jugement "
                f"({niveau_saisi}/5) et vos dispositifs ({niveau_dmr}/5).")
    else:
        return ('danger',
                f"🚨 Écart important ({ecart:.1f}) ! Votre jugement "
                f"({niveau_saisi}/5) ne reflète pas vos dispositifs ({niveau_dmr}/5).")


def calculer_scores_risque(impact, probabilite, niveau_maitrise):
    """
    Calcule les scores brut et net conformément à ISO 31000 / 31010.

    Returns dict avec :
        score_brut, niveau_brut,
        score_net, niveau_net,
        probabilite_ajustee, taux_reduction
    """
    if not impact or not probabilite:
        return {
            'score_brut': 0, 'niveau_brut': 'Non évalué',
            'score_net': 0, 'niveau_net': 'Non évalué',
            'probabilite_ajustee': 0, 'taux_reduction': 0
        }

    # Risque BRUT
    score_brut = impact * probabilite
    niveau_brut, _ = _qualifier_score(score_brut)

    # Ajustement maîtrise
    taux_reduction = get_taux_reduction(niveau_maitrise)
    probabilite_ajustee = round(probabilite * (1 - taux_reduction), 2)
    score_net = round(impact * probabilite_ajustee, 2)
    niveau_net, _ = _qualifier_score(score_net)

    return {
        'score_brut': score_brut,
        'niveau_brut': niveau_brut,
        'score_net': score_net,
        'niveau_net': niveau_net,
        'probabilite_ajustee': probabilite_ajustee,
        'taux_reduction': taux_reduction,
    }


def detecter_anomalies_risque(risque):
    """
    Détecte les anomalies d'un risque DANS LA CAMPAGNE ACTIVE uniquement.
    
    RÈGLES :
        1. Ignorer totalement les autres campagnes
        2. Si pas d'éval dans campagne active → pas d'anomalie
        3. Si Phase 1 seule → pas d'anomalie
        4. Si Phase 2 (validée, non confirmée) → anomalie "à confirmer"
        5. Si Phase 3 confirmée → pas d'anomalie
        6. Si Risque Critique/Élevé sans dispositif → anomalie
    
    Returns:
        list[dict]: [{niveau, message, action, label}, ...]
    """
    from datetime import datetime, timedelta
    ...
    # ============================================
    from models import CampagneEvaluation, EvaluationRisque

    campagne_active = None
    if risque.cartographie_id:
        campagne_active = CampagneEvaluation.query\
            .filter_by(
    ...
    # Pas de campagne active → pas d'anomalie possible
    if not campagne_active:
        return anomalies

    # ============================================
    # 2. RÉCUPÉRER L'ÉVAL DE LA CAMPAGNE ACTIVE
    # ============================================
    evaluation = EvaluationRisque.query\
        .filter_by(
            risque_id=risque.id,
            campagne_id=campagne_active.id
        )\
        .order_by(EvaluationRisque.created_at.desc())\
        .first()

    # ------------------------------------------------------------
    # CAS 1 : Pas d'évaluation dans la campagne active → OK
    # ------------------------------------------------------------
    if not evaluation:
        # On ne signale PAS — c'est un risque à évaluer, normal
        pass

    # ------------------------------------------------------------
    # CAS 2 : Évaluation existe → vérifier son état
    # ------------------------------------------------------------
    else:
        ...
        if evaluation.date_confirmation:
            phase = 'termine'
        elif evaluation.statut_validation == 'a_revoir':
            phase = 'a_revoir'
        elif evaluation.date_validation:
            phase = 'phase2'
        elif evaluation.date_pre_evaluation:
            phase = 'phase1'

        # 🔴 Anomalie UNIQUEMENT si Phase 2 (validée mais non confirmée)
        if phase == 'phase2':
            anomalies.append({
                'niveau': 'warning',
                'message': f"Évaluation validée mais non confirmée (Phase 2 → 3 requise).",
                'action': None,
                'label': "Confirmer"
            })

        # 🔴 Anomalie si "à revoir" (retour opérateur en attente)
        elif phase == 'a_revoir':
            anomalies.append({
                'niveau': 'danger',
                'message': f"Évaluation renvoyée à l'opérateur — en attente de correction.",
                'action': None,
                'label': "Voir"
            })

        # Phase 1 (pré-évaluée) → PAS d'anomalie
        # Phase 3 (confirmée) → PAS d'anomalie
        # Sinon → PAS d'anomalie

    # ============================================
    # 3. VÉRIFIER LES DISPOSITIFS (Critique/Élevé sans dispositif)
    # ============================================
    # Uniquement si le risque est Élevé/Critique NET
    if evaluation:
        niveau = evaluation.niveau_risque_net or evaluation.niveau_risque

        if niveau in ('Critique', 'Élevé'):
            nb_dispositifs = len([
                d for d in (risque.dispositifs_maitrise or [])
            ...
            if nb_dispositifs == 0:
                anomalies.append({
                    'niveau': 'warning',
                    'message': f"Risque {niveau} sans aucun dispositif de maîtrise.",
                    'action': None,
                    'label': "Créer un dispositif"
                })

    return anomalies


def generer_matrice_classique_propre(evaluations):
    """
    Matrice CLASSIQUE — Reprise de l'initiale.
    Points : taille CONSTANTE, 1 seul cercle + contour blanc épais.
    Étiquettes : à côté du point, discrètes.
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    COULEURS_POINTS = {
        'Faible':   '#059669',   # vert foncé
        'Moyen':    '#d97706',   # ambre foncé
        'Élevé':    '#ea580c',   # orange foncé
        'Critique': '#b91c1c',   # rouge foncé
    }
    ...
    # GRILLE
    # ============================================
    for i, impact in enumerate(np.arange(1, 6)):
        for j, proba in enumerate(np.arange(1, 6)):
            score = impact * proba
            niveau = _niveau(score)
            couleur = COULEURS_FOND[niveau]
    ...
    # ============================================
    # GROUPEMENT DES RISQUES PAR POSITION
    # ============================================
    positions = {}
    for evaluation in evaluations:
        impact = (evaluation.impact_conf or evaluation.impact_val or evaluation.impact_pre)
        proba = (evaluation.probabilite_conf or evaluation.probabilite_val or evaluation.probabilite_pre)

        if not (impact and proba and impact > 0 and proba > 0):
            continue

        key = (impact, proba)
        positions.setdefault(key, []).append(evaluation)

    # ============================================
    # TRACÉ DES POINTS
    # ============================================
    for (impact, proba), evals_here in positions.items():
        x_centre = (proba - 1) + 0.5
        y_centre = (impact - 1) + 0.5

        if len(evals_here) == 1:
            # ========== UN SEUL RISQUE ==========
            evaluation = evals_here[0]
            niveau = evaluation.niveau_risque_net or evaluation.niveau_risque or 'Faible'
            couleur = COULEURS_POINTS.get(niveau, '#666666')

            # ✅ UN SEUL POINT : couleur unie + contour blanc épais
            ax.plot(x_centre, y_centre, 'o',
                    markersize=22,
                    color=couleur,
            ...
            # Référence au centre (texte blanc)
            ax.text(x_centre, y_centre,
                    evaluation.risque.reference,
                    ha='center', va='center',
                    fontsize=8, fontweight='bold',
                    color='white', zorder=7)

        else:
            # ========== PLUSIEURS RISQUES À LA MÊME POSITION ==========
            # Répartir en cercle autour du centre
            n = len(evals_here)
            rayon = 0.22 if n <= 3 else 0.26
            angle_step = 2 * np.pi / n

            for k, evaluation in enumerate(evals_here):
                angle = k * angle_step - np.pi / 2
                x = x_centre + rayon * np.cos(angle)
                y = y_centre + rayon * np.sin(angle)

                niveau = evaluation.niveau_risque_net or evaluation.niveau_risque or 'Faible'
                couleur = COULEURS_POINTS.get(niveau, '#666666')

                # Point individuel plus petit
                ax.plot(x, y, 'o',
                        markersize=14,
                        color=couleur,
                        markeredgecolor='white',
                ...
                # Petite étiquette à côté (référence abrégée)
                ax.annotate(
                    evaluation.risque.reference,
                    (x, y),
                    xytext=(10, 6),
    ...
    ax.set_title(
        "Matrice des Risques — Classique (NET)",
    ...
    # ============================================
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Faible (1-4)',
               markerfacecolor=COULEURS_POINTS['Faible'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Moyen (5-10)',
               markerfacecolor=COULEURS_POINTS['Moyen'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Élevé (11-16)',
               markerfacecolor=COULEURS_POINTS['Élevé'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Critique (17-25)',
               markerfacecolor=COULEURS_POINTS['Critique'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
    ]
    ...
    print(f"✅ Matrice CLASSIQUE générée ({len(evaluations)} risques)")
    return graphic


def generer_matrice_criticite_propre(evaluations):
    """
    Matrice CRITICITÉ — Bulles SUBTILES (taille 14 → 22 px).
    Taille = BRUT mais plafonnée pour rester lisible.
    Couleur remplissage = BRUT, contour = NET.
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    for evaluation in evaluations:
        ...
        score_brut = evaluation.score_risque_brut or (impact * proba)
        score_net = evaluation.score_risque_net or evaluation.score_risque or score_brut
        niveau_brut = evaluation.niveau_risque_brut or _niveau(score_brut)
        niveau_net = evaluation.niveau_risque_net or evaluation.niveau_risque or _niveau(score_net)

        risques_valides.append({
    ...
    # Trier : petits d'abord
    risques_valides.sort(key=lambda r: r['score_brut'])

    # ============================================
    # TRACÉ (bulles subtiles)
    # ============================================
    for r in risques_valides:
        x = (r['proba'] - 1) + 0.5
        y = (r['impact'] - 1) + 0.5

        # ✅ Taille modérée : 16 → 26 px (au lieu de 30→60)
        # Formule : 16 + (score_brut / 25) * 10
        taille = 16 + (r['score_brut'] / 25.0) * 10

        couleur_brut = COULEURS_POINTS.get(r['niveau_brut'], '#666666')
        couleur_net = COULEURS_POINTS.get(r['niveau_net'], '#666666')

        # UN SEUL plot : remplissage = BRUT, contour = NET
        ax.plot(x, y, 'o',
                markersize=taille,
                color=couleur_brut,
                markeredgecolor=couleur_net,
                markeredgewidth=3,
                alpha=0.85,
                zorder=6)

        # Référence au centre
        ax.text(x, y, r['evaluation'].risque.reference,
                ha='center', va='center',
                fontsize=8, fontweight='bold',
                color='white', zorder=7)
    ...
    ax.set_title(
        "Matrice de Criticité — Taille ∝ BRUT · Couleur = BRUT · Contour = NET",
    ...
    # ============================================
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Faible',
               markerfacecolor=COULEURS_POINTS['Faible'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Moyen',
               markerfacecolor=COULEURS_POINTS['Moyen'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Élevé',
               markerfacecolor=COULEURS_POINTS['Élevé'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Critique',
               markerfacecolor=COULEURS_POINTS['Critique'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
    ]
    ...
    print(f"✅ Matrice CRITICITÉ générée ({len(risques_valides)} risques)")
    return graphic

def generer_matrice_priorisation_propre(evaluations):
    """
    Matrice PRIORISATION — Points constants (18 px), contraste franc.
    Quadrants colorés avec labels clairs.
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    ax.add_patch(patches.Rectangle(
        (0, SEUIL_MAITRISE), SEUIL_NET, 5.5 - SEUIL_MAITRISE,
        facecolor=FOND_QUADRANT['sous_controle'],
        edgecolor='none', zorder=1
    ))
    ax.add_patch(patches.Rectangle(
        (SEUIL_NET, SEUIL_MAITRISE), 25.5 - SEUIL_NET, 5.5 - SEUIL_MAITRISE,
        facecolor=FOND_QUADRANT['surveiller'],
        edgecolor='none', zorder=1
    ))
    ax.add_patch(patches.Rectangle(
        (0, 0.5), SEUIL_NET, SEUIL_MAITRISE - 0.5,
        facecolor=FOND_QUADRANT['ameliorer'],
        edgecolor='none', zorder=1
    ))
    ax.add_patch(patches.Rectangle(
        (SEUIL_NET, 0.5), 25.5 - SEUIL_NET, SEUIL_MAITRISE - 0.5,
        facecolor=FOND_QUADRANT['urgent'],
        edgecolor='none', zorder=1
    ))

    # Lignes de séparation
    ax.axvline(x=SEUIL_NET, color='#374151', linestyle='--',
               linewidth=2, alpha=0.6, zorder=2)
    ax.axhline(y=SEUIL_MAITRISE, color='#374151', linestyle='--',
               linewidth=2, alpha=0.6, zorder=2)

    # ============================================
    # LABELS QUADRANTS
    # ============================================
    ax.text(SEUIL_NET / 2, 5.35, '✅ SOUS CONTRÔLE',
            ha='center', va='top',
            fontsize=13, fontweight='bold', color='#065f46', zorder=3)
    ax.text(SEUIL_NET / 2, 5.08, 'NET faible + maîtrise bonne',
            ha='center', va='top',
            fontsize=9, color='#065f46', alpha=0.8, style='italic', zorder=3)

    ax.text((SEUIL_NET + 25) / 2, 5.35, '⚠️ SURVEILLER',
            ha='center', va='top',
            fontsize=13, fontweight='bold', color='#92400e', zorder=3)
    ax.text((SEUIL_NET + 25) / 2, 5.08, 'NET élevé malgré bonne maîtrise',
            ha='center', va='top',
            fontsize=9, color='#92400e', alpha=0.8, style='italic', zorder=3)

    ax.text(SEUIL_NET / 2, 0.65, '💡 AMÉLIORER',
            ha='center', va='bottom',
            fontsize=13, fontweight='bold', color='#1e40af', zorder=3)
    ax.text(SEUIL_NET / 2, 0.9, 'NET faible + maîtrise faible',
            ha='center', va='bottom',
            fontsize=9, color='#1e40af', alpha=0.8, style='italic', zorder=3)

    ax.text((SEUIL_NET + 25) / 2, 0.65, '🚨 URGENT',
            ha='center', va='bottom',
            fontsize=13, fontweight='bold', color='#991b1b', zorder=3)
    ax.text((SEUIL_NET + 25) / 2, 0.9, 'NET élevé + maîtrise faible',
            ha='center', va='bottom',
            fontsize=9, color='#991b1b', alpha=0.8, style='italic', zorder=3)

    # ============================================
    # POINTS (taille constante)
    # ============================================
    for evaluation in evaluations:
        impact = (evaluation.impact_conf or evaluation.impact_val or evaluation.impact_pre)
        proba = (evaluation.probabilite_conf or evaluation.probabilite_val or evaluation.probabilite_pre)
        ...
        score_net = (evaluation.score_risque_net or evaluation.score_risque
                     or (impact * proba))
        maitrise = (evaluation.niveau_maitrise_conf
                    or evaluation.niveau_maitrise_val
                    or evaluation.niveau_maitrise_pre or 3)

        # Quadrant
        if score_net <= SEUIL_NET and maitrise >= SEUIL_MAITRISE:
            quadrant = 'sous_controle'
        elif score_net > SEUIL_NET and maitrise >= SEUIL_MAITRISE:
            quadrant = 'surveiller'
        elif score_net <= SEUIL_NET and maitrise < SEUIL_MAITRISE:
            quadrant = 'ameliorer'
        else:
        ...
        couleur = COULEURS_QUADRANT[quadrant]

        # ✅ Point de taille CONSTANTE et MODÉRÉE
        ax.plot(score_net, maitrise, 'o',
                markersize=22,
                color=couleur,
                markeredgecolor='white',
                markeredgewidth=3,
    ...
    ax.tick_params(axis='x', labelsize=11, colors='#374151', length=4)
    ax.tick_params(axis='y', labelsize=11, colors='#374151', length=4)
    ...
    ax.set_xlabel('Score NET (risque résiduel)',
                  fontsize=13, fontweight='bold',
                  color='#1a2a5e', labelpad=10)
    ax.set_ylabel('Niveau de maîtrise',
                  fontsize=13, fontweight='bold',
                  color='#1a2a5e', labelpad=10)

    ax.set_title(
        "Matrice de Priorisation — Plan d'action",
    ...
    # ============================================
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Sous contrôle',
               markerfacecolor=COULEURS_QUADRANT['sous_controle'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Surveiller',
               markerfacecolor=COULEURS_QUADRANT['surveiller'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Améliorer',
               markerfacecolor=COULEURS_QUADRANT['ameliorer'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Urgent',
               markerfacecolor=COULEURS_QUADRANT['urgent'],
               markersize=14, markeredgecolor='white', markeredgewidth=2),
    ]
    ...
    print(f"✅ Matrice PRIORISATION générée ({len(evaluations)} risques)")
    return graphic
...
def generer_matrice_risques(evaluations, matrice_type='classique'):
    """
    Dispatcher vers la matrice appropriée.
    Types : 'classique' | 'criticite' | 'priorisation'
    """
    if matrice_type == 'classique':
        return generer_matrice_classique_propre(evaluations)
    elif matrice_type == 'criticite':
        return generer_matrice_criticite_propre(evaluations)
    elif matrice_type == 'priorisation':
        return generer_matrice_priorisation_propre(evaluations)
    else:
        print(f"⚠️ Type inconnu: {matrice_type}, fallback classique")
        return generer_matrice_classique_propre(evaluations)

def _generer_matrice_classique_legacy(evaluations, matrice_type='classique'):
    """
    Génère 4 matrices distinctes :
    - classique    : x=probabilité, y=impact, couleur=niveau NET
    - criticite    : x=probabilité, y=impact, taille du point=score BRUT
    - priorisation : x=maîtrise, y=score NET, couleur=niveau NET
    - (surbrillance est gérée par generer_matrice_risque_specifique)
    """
    ...
    evaluations_valides = []
    colors = ['blue', 'purple', 'darkred', 'darkgreen', 'darkorange',
              'darkcyan', 'brown', 'pink', 'navy', 'teal']
    
    for idx, evaluation in enumerate(evaluations):
        impact = (evaluation.impact_conf or evaluation.impact_val or evaluation.impact_pre)
        proba = (evaluation.probabilite_conf or evaluation.probabilite_val or evaluation.probabilite_pre)
        maitrise = (evaluation.niveau_maitrise_conf or evaluation.niveau_maitrise_val
                    or evaluation.niveau_maitrise_pre or 3)
        
        if not (impact and proba and impact > 0 and proba > 0):
            continue
        
        # Coordonnées selon type
        if matrice_type == 'priorisation':
            # X = maîtrise (1→5), Y = score NET
            score_net = evaluation.score_risque_net or evaluation.score_risque or (impact * proba)
            x = maitrise - 1
            y = min(4, max(0, score_net / 6.0))  # score 0-25 → y 0-4
        else:
            # classique & criticite : x=proba, y=impact
            x = proba - 1
            y = impact - 1
        ...
        risques_par_position.setdefault(position_key, []).append({
            'evaluation': evaluation,
            'x_centre': x_centre, 'y_centre': y_centre,
            'x': x, 'y': y,
            'impact': impact, 'probabilite': proba,
            'score': impact * proba,
            'index': idx
        })
    ...
    for position_key, risques_list in risques_par_position.items():
        if matrice_type == 'criticite':
            # Taille du point = score BRUT
            for r in risques_list:
                color_idx = r['index'] % len(colors)
                score_brut = (r['evaluation'].score_risque_brut
                              or r['impact'] * r['probabilite'])
                taille = 80 + (score_brut / 25.0) * 400
                ax.plot(r['x_centre'], r['y_centre'], 'o', markersize=taille**0.5,
                        color=colors[color_idx], markeredgecolor='white',
                        markeredgewidth=2, alpha=0.85, zorder=5)
                ax.annotate(r['evaluation'].risque.reference,
                            (r['x_centre'], r['y_centre']),
            ...
            r = risques_list[0]
            color_idx = r['index'] % len(colors)
            ax.plot(r['x_centre'], r['y_centre'], 'o', markersize=16,
                    color=colors[color_idx], markeredgecolor='white',
                    markeredgewidth=3, alpha=0.9, zorder=5)
            ax.annotate(r['evaluation'].risque.reference,
                        (r['x_centre'], r['y_centre']),
                        xytext=(12, 12), textcoords='offset points',
            ...
            for i, r in enumerate(risques_list):
                color_idx = r['index'] % len(colors)
                angle = i * angle_step
                ...
                ax.annotate(r['evaluation'].risque.reference, (x, y),
    ...
    if matrice_type == 'classique':
        ...
        ax.set_xlabel('Probabilité', fontsize=13, fontweight='bold', labelpad=15)
        ax.set_ylabel('Impact', fontsize=13, fontweight='bold', labelpad=15)
        titre = "Matrice des Risques — Classique (NET)"
    elif matrice_type == 'criticite':
        ax.set_xticklabels(['1\nTrès rare', '2\nRare', '3\nPossible', '4\nProbable', '5\nTrès probable'], fontsize=11)
        ax.set_yticklabels(['1\nNégligeable', '2\nMineur', '3\nModéré', '4\nImportant', '5\nCritique'], fontsize=11)
        ax.set_xlabel('Probabilité', fontsize=13, fontweight='bold', labelpad=15)
        ax.set_ylabel('Impact', fontsize=13, fontweight='bold', labelpad=15)
        titre = "Matrice de Criticité — taille = score BRUT"
    elif matrice_type == 'priorisation':
        ax.set_xticks([0.5, 1.5, 2.5, 3.5, 4.5])
        ax.set_xticklabels(['1\nInsuffisant', '2\nPartiel', '3\nAdéquat', '4\nBon', '5\nExcellent'], fontsize=11)
        ax.set_yticks([0.5, 1.5, 2.5, 3.5, 4.5])
        ax.set_yticklabels(['0-5', '5-10', '10-15', '15-20', '20-25'], fontsize=11)
        ax.set_xlabel('Niveau de Maîtrise', fontsize=13, fontweight='bold', labelpad=15)
        ax.set_ylabel('Score NET', fontsize=13, fontweight='bold', labelpad=15)
        titre = "Matrice de Priorisation — NET vs Maîtrise"
    else:
        titre = f"Matrice — {matrice_type}"
    
    ax.set_title(titre, fontsize=16, fontweight='bold', pad=25)
    
    # Légende
    legend_x = 5.2
    ax.text(legend_x, 4.5, 'LÉGENDE:', fontweight='bold', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", fc='lightblue', alpha=0.7))
    niveaux = [('Faible (1-4)', 'lightgreen'), ('Moyen (5-10)', 'yellow'),
               ('Élevé (11-16)', 'orange'), ('Critique (17-25)', 'red')]
    for i, (texte, couleur) in enumerate(niveaux):
        ax.add_patch(patches.Rectangle((legend_x, 4.1 - i*0.2), 0.3, 0.15, facecolor=couleur))
        ax.text(legend_x + 0.4, 4.17 - i*0.2, texte, fontsize=8, va='center')
    ...
    print(f"✅ Matrice {matrice_type} générée ({len(evaluations_valides)} risques)")
    return graphic
...
def generer_matrice_risque_specifique(evaluations, risque_surbrillance=None):
    """
    Génère une matrice avec un risque spécifique en surbrillance.
    
    Args:
        evaluations: Liste des évaluations à afficher
        risque_surbrillance: Risque à mettre en évidence
    ...
    for i in range(5):
        for j in range(5):
            score_cell = (i + 1) * (j + 1)
            niveau = _niveau(score_cell)
            couleur = COULEURS_FOND[niveau]

            rect = patches.Rectangle(
                (j, i), 1, 1,
                linewidth=2, edgecolor='black',
                facecolor=couleur, alpha=0.7
            )
            ax.add_patch(rect)

            ax.text(j + 0.5, i + 0.5, f'{score_cell}',
                    ha='center', va='center',
                    fontsize=14, fontweight='bold')

    # Tracé des risques
    for evaluation in evaluations:
        impact = (evaluation.impact_conf or evaluation.impact_val 
                  or evaluation.impact_pre)
        proba = (evaluation.probabilite_conf or evaluation.probabilite_val 
                 or evaluation.probabilite_pre)
        ...
        # Est-ce le risque cible ?
        is_target = (
            risque_surbrillance 
            and evaluation.risque_id == risque_surbrillance.id
        )

        if is_target:
            # Risque ciblé : gros marqueur rouge avec halo
            ax.plot(x, y, 'o', markersize=25, color='red',
                    markeredgecolor='yellow', markeredgewidth=4,
                    alpha=1.0, zorder=10)

            circle = plt.Circle((x, y), 0.35, color='yellow', 
                                alpha=0.3, zorder=9)
            ax.add_patch(circle)

            ax.annotate(
                f"⭐ {evaluation.risque.reference} ⭐",
                (x, y),
                xytext=(25, 25),
                textcoords='offset points',
                fontsize=12, fontweight='bold',
                bbox=dict(boxstyle="round,pad=0.5", 
                          fc='yellow', alpha=0.9, ec='orange'),
                arrowprops=dict(arrowstyle='->', color='orange', lw=2),
                zorder=11
            )
        else:
            # Autres risques : petit marqueur bleu
    ...
    # Légende
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Risque ciblé',
               markerfacecolor='red', markersize=14,
               markeredgecolor='yellow', markeredgewidth=2),
        Line2D([0], [0], marker='o', color='w', label='Autres risques',
               markerfacecolor='#3b82f6', markersize=12,
               markeredgecolor='white', markeredgewidth=2),
    ]
    ...
    print(f"✅ Matrice spécifique générée ({len(evaluations)} évals, "
          f"cible={risque_surbrillance.reference if risque_surbrillance else 'N/A'})")
    return graphic
