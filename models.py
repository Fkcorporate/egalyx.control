...
class User(UserMixin, db.Model):
    ...
    def has_permission(self, permission):
        ...
        # ============================================
        # 6. ✅ PERMISSIONS MODULE VEILLE RÉGLEMENTAIRE
        # (AVANT le return False final)
        # ============================================
        veille_permissions = {
            # Accès de base
            'can_view_veille': True,
            'can_manage_regulatory': self.role in ['admin', 'manager', 'compliance'] or getattr(self, 'is_client_admin', False),
            
            # Création / Édition
            'can_create_veille': self.role in ['admin', 'manager'] or getattr(self, 'is_client_admin', False),
            'can_edit_veille': self.role in ['admin', 'manager'] or getattr(self, 'is_client_admin', False),
            'can_delete_veille': self.role == 'admin' or getattr(self, 'is_client_admin', False),
            
            # Workflow
            'can_approve_veille': self.role in ['admin', 'manager'] or getattr(self, 'is_client_admin', False),
            
            # Gestion
            'can_manage_actions_veille': self.role in ['admin', 'manager'] or getattr(self, 'is_client_admin', False),
            'can_upload_documents_veille': self.role in ['admin', 'manager', 'auditeur'],
            'can_export_veille': self.role in ['admin', 'manager'] or getattr(self, 'is_client_admin', False),
            'can_view_audit_trail': self.role == 'admin' or getattr(self, 'is_client_admin', False),
        }
        
        if permission in veille_permissions:
            return veille_permissions[permission]
...
# ============================================
# ORGANISMES RÉGULATEURS (défini AVANT VeilleReglementaire)
# ============================================
# -------------------- VEILLE REGLEMENTAIRE --------------------

veille_risques = db.Table('veille_risques',
    db.Column('veille_id', db.Integer, 
              db.ForeignKey('veille_reglementaire.id', ondelete='CASCADE'), 
              primary_key=True),
    db.Column('risque_id', db.Integer, 
              db.ForeignKey('risques.id', ondelete='CASCADE'), 
              primary_key=True),
    db.Column('type_lien', db.String(50), default='impact'),
    db.Column('date_association', db.DateTime, default=datetime.utcnow),
    db.Column('associe_par', db.Integer, db.ForeignKey('user.id'))
)

veille_organismes = db.Table('veille_organismes',
    db.Column('veille_id', db.Integer, 
              db.ForeignKey('veille_reglementaire.id', ondelete='CASCADE'), 
              primary_key=True),
    db.Column('organisme_id', db.Integer, 
              db.ForeignKey('organismes_regulateurs.id', ondelete='CASCADE'), 
              primary_key=True),
    db.Column('date_association', db.DateTime, default=datetime.utcnow)
)
# ============================================
# TABLE D'ASSOCIATION : VEILLE ↔ PÔLES
# ============================================

veille_poles = db.Table('veille_poles',
    db.Column('veille_id', 
              db.Integer, 
              db.ForeignKey('veille_reglementaire.id', ondelete='CASCADE'), 
              primary_key=True),
    db.Column('pole_id', 
              db.Integer, 
              db.ForeignKey('poles.id', ondelete='CASCADE'), 
              primary_key=True),
    db.Column('impact', db.String(20), default='moyen'),  # faible, moyen, eleve
    db.Column('date_association', db.DateTime, default=datetime.utcnow)
)

class OrganismeRegulateur(db.Model):
    """Organisme émetteur de réglementations"""
    __tablename__ = 'organismes_regulateurs'
    
    id = db.Column(db.Integer, primary_key=True)
    code = db.Column(db.String(20), nullable=False)
    nom = db.Column(db.String(200), nullable=False)
    nom_complet = db.Column(db.String(300))
    description = db.Column(db.Text)
    ...
    # Relations
    veilles = db.relationship(
        'VeilleReglementaire',
        secondary=veille_organismes,
        back_populates='organismes'
    )
    createur = db.relationship('User', foreign_keys=[created_by])   # ✅ AJOUTER CETTE LIGNE
    
    __table_args__ = (
        db.UniqueConstraint('code', 'client_id', name='uix_organisme_code_client'),
    )
    
    def __repr__(self):
        return f'<Organisme {self.code}: {self.nom}>'

# -------------------- VEILLE REGLEMENTAIRE --------------------
class VeilleReglementaire(db.Model):
    """Veille réglementaire avec workflow d'archivage et multi-tenant"""
    __tablename__ = 'veille_reglementaire'
    ...
    archive_reason = db.Column(db.String(500), nullable=True)
    ...
    # Multi-tenant
    client_id = db.Column(db.Integer, db.ForeignKey('clients.id'), nullable=True)
    metadonnees = db.Column(db.JSON, default=dict)
    source_type = db.Column(db.String(50))  # 'legifrance', 'laws_africa', 'manuel'
    source_id = db.Column(db.String(255))   # JORFTEXT, LEGIARTI, FRBR URI...
    source_url = db.Column(db.String(500))  # URL directe vers la source
    source_hash = db.Column(db.String(64))  # SHA256 du contenu pour détecter les modifs
    date_derniere_sync = db.Column(db.DateTime)
    ...
    # Relations
    pays = db.relationship('Pays', foreign_keys=[pays_id], backref='veilles_reglementaires')
    pole = db.relationship('Pole', foreign_keys=[pole_id], backref='veilles_reglementaires')
    direction = db.relationship('Direction', foreign_keys=[direction_id], backref='veilles_reglementaires')
    service = db.relationship('Service', foreign_keys=[service_id], backref='veilles_reglementaires')
    
    @property
    def perimetre_applicable(self):
        """Retourne le périmètre d'application lisible"""
        if self.service_id and self.service:
            return f"Service : {self.service.nom}"
        elif self.direction_id and self.direction:
            return f"Direction : {self.direction.nom}"
        elif self.pole_id and self.pole:
            return f"Pôle : {self.pole.nom}"
        elif self.pays_id and self.pays:
            return f"Pays : {self.pays.nom}"
        return "Global (toute l'organisation)"
    
    @property
    def perimetre_type(self):
        """Retourne le type de périmètre"""
        if self.service_id:
            return 'service'
        elif self.direction_id:
            return 'direction'
        elif self.pole_id:
            return 'pole'
        elif self.pays_id:
    ...
    @property
    def perimetre_couleur(self):
        """Retourne la couleur associée au périmètre"""
        couleurs = {
            'service': 'info',
            'direction': 'primary',
            'pole': 'warning',
        ...
        return couleurs.get(self.perimetre_type, 'secondary')
    # ============================================
    # RELATIONS
    # ============================================
    createur = db.relationship(
        'User', 
        back_populates='veilles_crees', 
        foreign_keys=[created_by]
    )
    archiviste = db.relationship(
        'User', 
        foreign_keys=[archived_by]
    )
    actions = db.relationship(
        'ActionConformite', 
        back_populates='veille', 
        lazy='dynamic',
        cascade='all, delete-orphan'
    )
    ...
    workflow_approbation = db.relationship(
        'VeilleWorkflowApprobation',
        back_populates='veille',
        uselist=False,
        cascade='all, delete-orphan'
    )
    organismes = db.relationship(
        'OrganismeRegulateur',
        secondary=veille_organismes,
        back_populates='veilles',
        lazy='select'
    )
    risques_associes = db.relationship(
        'Risque',
        secondary=veille_risques,
        backref=db.backref('veilles_associees', lazy='dynamic'),
        lazy='select'
    )
    poles_associes = db.relationship(
        'Pole',
        secondary=veille_poles,
        backref=db.backref('veilles_associees', lazy='dynamic'),
        lazy='select'
    )
    def creer_risque_depuis_veille(self, user_id, cartographie_id=None):
        """Crée automatiquement un risque à partir de cette veille"""
        from models import Risque
        
        annee = datetime.now().year
        count = Risque.query.filter(Risque.reference.like(f'RSQ-{annee}-%')).count()
        reference = f'RSQ-{annee}-{(count + 1):04d}'
        
        risque = Risque(
            reference=reference,
            intitule=f"Non-conformité à {self.titre}",
            description=(
                f"Risque de non-conformité à la réglementation '{self.titre}' "
                f"({self.reference or 'sans réf'}).\n\n{self.description or ''}"
            ),
            categorie='Conformité',
            type_risque='Réglementaire',
    ...
    # ============================================
    @property
    def organismes_noms(self):
        """Retourne les noms des organismes séparés par des virgules"""
        if not self.organismes:
            return self.organisme_emetteur or '-'
        return ', '.join([o.nom for o in self.organismes])
    @property
    def actions_actives(self):
        """Retourne les actions actives (non archivées)"""
        return self.actions.filter_by(is_archived=False).all()
    
    @property
    def actions_archivees(self):
        """Retourne les actions archivées"""
        return self.actions.filter_by(is_archived=True).all()
    
    @property
    def nb_actions_actives(self):
        """Nombre d'actions actives"""
        return self.actions.filter_by(is_archived=False).count()
    
    @property
    def nb_actions_archivees(self):
        """Nombre d'actions archivées"""
        return self.actions.filter_by(is_archived=True).count()
    ...
    @property
    def days_until_application(self):
        """Nombre de jours avant application"""
        if not self.date_application:
            return None
        return (self.date_application - datetime.utcnow().date()).days
    ...
    def impact_label(self):
        ...
        return labels.get(self.impact_estime, '⚪ Non évalué')
    
    @property
    def impact_css(self):
        """Classe CSS pour l'impact"""
        css = {
            'eleve': 'danger',
            'moyen': 'warning',
            'faible': 'success'
        }
        return css.get(self.impact_estime, 'secondary')
    
    __table_args__ = (
        db.Index('ix_veille_client_statut', 'client_id', 'statut'),
        db.Index('ix_veille_date_application', 'date_application'),
        db.Index('ix_veille_archive_active', 'is_archived', 'is_active'),
        db.Index('ix_veille_impact', 'impact_estime'),
    )
    ...
    def __repr__(self):
        return f'<VeilleReglementaire {self.id}: {self.titre[:50]}>'

# ============================================
# MENACES IGNORÉES (historique)
# ============================================

class MenaceIgnoree(db.Model):
    """Historique des menaces ignorées par l'utilisateur"""
    __tablename__ = 'menaces_ignorees'
    
    id = db.Column(db.Integer, primary_key=True)
    hash_menace = db.Column(db.String(64), nullable=False, index=True)
    titre = db.Column(db.String(500))
    source = db.Column(db.String(50))
    raison_ignore = db.Column(db.Text)
    
    client_id = db.Column(db.Integer, db.ForeignKey('clients.id'), nullable=False)
    ignore_par = db.Column(db.Integer, db.ForeignKey('user.id'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    client = db.relationship('Client', foreign_keys=[client_id])
    utilisateur = db.relationship('User', foreign_keys=[ignore_par])
    
    __table_args__ = (
        db.UniqueConstraint('hash_menace', 'client_id', name='uix_menace_ignoree'),
    )
    
    def __repr__(self):
        return f'<MenaceIgnoree {self.hash_menace}>'

class VeilleCommentaire(db.Model):
    """Commentaires collaboratifs sur une veille"""
    __tablename__ = 'veille_commentaires'
    
    id = db.Column(db.Integer, primary_key=True)
    veille_id = db.Column(db.Integer, db.ForeignKey('veille_reglementaire.id'), nullable=False)
    utilisateur_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    contenu = db.Column(db.Text, nullable=False)
    parent_id = db.Column(db.Integer, db.ForeignKey('veille_commentaires.id'))
    ...
    veille = db.relationship('VeilleReglementaire', backref='commentaires_collab')
    utilisateur = db.relationship('User')
    parent = db.relationship('VeilleCommentaire', remote_side=[id], backref='reponses')

class MatriceConformite(db.Model):
    """Matrice de conformité veille × organisation"""
    __tablename__ = 'matrice_conformite'
    ...
    statut_conformite = db.Column(db.String(20))  # conforme, partiel, non_conforme, na
    commentaire = db.Column(db.Text)
    preuves = db.Column(db.Text)
    
    evalue_par = db.Column(db.Integer, db.ForeignKey('user.id'))
    date_evaluation = db.Column(db.DateTime, default=datetime.utcnow)
    prochaine_evaluation = db.Column(db.Date)
    
    client_id = db.Column(db.Integer, db.ForeignKey('clients.id'))
    
    veille = db.relationship('VeilleReglementaire', backref='matrices_conformite')
    direction = db.relationship('Direction')
    service = db.relationship('Service')
    evaluateur = db.relationship('User', foreign_keys=[evalue_par])
class VeilleFavori(db.Model):
    """Favoris utilisateur"""
    __tablename__ = 'veille_favoris'
    ...
    veille = db.relationship('VeilleReglementaire', backref='favoris_users')
    utilisateur = db.relationship('User')
    
    __table_args__ = (
        db.UniqueConstraint('veille_id', 'utilisateur_id', name='uix_favori'),
    )
    
# -------------------- ACTION CONFORMITE --------------------
class ActionConformite(db.Model):
    """Action de conformité liée à une veille"""
    __tablename__ = 'action_conformite'
    ...
    responsable = db.relationship(
        'User', 
        back_populates='actions_conformite', 
        foreign_keys=[responsable_id]
    )
    archiviste = db.relationship('User', foreign_keys=[archived_by])
    __table_args__ = (
        db.Index('ix_action_veille_statut', 'veille_id', 'statut'),
        db.Index('ix_action_echeance', 'date_echeance'),
        db.Index('ix_action_archived', 'is_archived'),
    )
    # ============================================
    # PROPRIÉTÉS CALCULÉES
    # ============================================
    ...
    def est_en_retard(self):
        ...
        if not self.date_echeance:
            return False
        return self.date_echeance < datetime.utcnow().date()
    ...
    def jours_retard(self):
        ...
        if not self.est_en_retard:
            return 0
        return (datetime.utcnow().date() - self.date_echeance).days
    
    @property
    def jours_restants(self):
        """Nombre de jours restants avant échéance"""
        if self.statut == 'termine' or not self.date_echeance:
            return None
        delta = (self.date_echeance - datetime.utcnow().date()).days
        return delta if delta >= 0 else 0
    
    # ============================================
    # MÉTHODES D'ARCHIVAGE
    # ============================================
    def archiver(self, user_id=None):
        """Archive l'action"""
        self.is_archived = True
        self.is_active = False
        self.archived_at = datetime.utcnow()
    ...
    def __repr__(self):
        return f'<ActionConformite {self.id}: {self.description[:30]}>'

# ============================================
# WORKFLOW D'APPROBATION VEILLE
# ============================================

class VeilleWorkflowApprobation(db.Model):
    """Workflow d'approbation d'une veille réglementaire"""
    __tablename__ = 'veille_workflow_approbation'
    ...
    # Étape actuelle
    etape_actuelle = db.Column(db.String(50), default='brouillon')
    # Valeurs : brouillon, en_relecture, approuvee, en_vigueur, rejetee
    
    # Approbateurs désignés
    approbateur_niveau1_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    approbateur_niveau2_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    
    # Dates clés
    date_envoi_relecture = db.Column(db.DateTime)
    date_approbation_n1 = db.Column(db.DateTime)
    date_approbation_n2 = db.Column(db.DateTime)
    date_rejet = db.Column(db.DateTime)
    
    # Commentaires
    commentaire_relecture = db.Column(db.Text)
    commentaire_approbation_n1 = db.Column(db.Text)
    commentaire_approbation_n2 = db.Column(db.Text)
    commentaire_rejet = db.Column(db.Text)
    
    # Historique JSON
    historique_etapes = db.Column(db.JSON, default=list)
    ...
    # Relations
    veille = db.relationship(
        'VeilleReglementaire', 
        back_populates='workflow_approbation'
    )
    approbateur_n1 = db.relationship('User', foreign_keys=[approbateur_niveau1_id])
    approbateur_n2 = db.relationship('User', foreign_keys=[approbateur_niveau2_id])
    createur = db.relationship('User', foreign_keys=[created_by])
    ...
    def envoyer_en_relecture(self, user_id, commentaire=None):
        if self.etape_actuelle != 'brouillon':
            return False, "Seul un brouillon peut être envoyé en relecture"
        
        ancien = self.etape_actuelle
        self.etape_actuelle = 'en_relecture'
        self.date_envoi_relecture = datetime.utcnow()
        self.commentaire_relecture = commentaire
        self._ajouter_historique(ancien, self.etape_actuelle, user_id, commentaire)
        self.updated_at = datetime.utcnow()
        return True, "Veille envoyée en relecture"
    
    def approuver_niveau1(self, user_id, commentaire=None):
        if self.etape_actuelle != 'en_relecture':
            return False, "La veille doit être en relecture"
        
        ancien = self.etape_actuelle
        self.etape_actuelle = 'approuvee'
        self.date_approbation_n1 = datetime.utcnow()
        self.approbateur_niveau1_id = user_id
        self.commentaire_approbation_n1 = commentaire
        self._ajouter_historique(ancien, self.etape_actuelle, user_id, commentaire)
        self.updated_at = datetime.utcnow()
        return True, "Veille approuvée niveau 1"
    
    def approuver_niveau2(self, user_id, commentaire=None):
        if self.etape_actuelle != 'approuvee':
            return False, "La veille doit être approuvée niveau 1"
        
        ancien = self.etape_actuelle
        self.etape_actuelle = 'en_vigueur'
        self.date_approbation_n2 = datetime.utcnow()
        self.approbateur_niveau2_id = user_id
        self.commentaire_approbation_n2 = commentaire
        self._ajouter_historique(ancien, self.etape_actuelle, user_id, commentaire)
        self.updated_at = datetime.utcnow()
        
        # Mettre à jour le statut de la veille
        if self.veille:
            self.veille.statut = 'en_vigueur'
        
        return True, "Veille approuvée niveau 2"
    
    def rejeter(self, user_id, raison):
        if self.etape_actuelle not in ['en_relecture', 'approuvee']:
            return False, "Aucune relecture en cours"
        
        if not raison or len(raison.strip()) < 5:
            return False, "Veuillez fournir une raison (min 5 caractères)"
        
        ancien = self.etape_actuelle
        self.etape_actuelle = 'rejetee'
        self.date_rejet = datetime.utcnow()
        self.commentaire_rejet = raison
        self._ajouter_historique(ancien, self.etape_actuelle, user_id, raison)
        self.updated_at = datetime.utcnow()
        return True, "Veille rejetée"
    
    # ============================================
    # PERMISSIONS
    # ============================================
    
    def peut_approuver_n1(self, user):
        if self.etape_actuelle != 'en_relecture':
            return False
        
        veille = self.veille
        # Le manager du pôle concerné
        if veille.pole and veille.pole.responsable_id == user.id:
            return True
        # Le manager de la direction concernée
        if veille.direction and veille.direction.responsable_id == user.id:
            return True
        # L'admin client
    ...
    def peut_approuver_n2(self, user):
        if self.etape_actuelle != 'approuvee':
            return False
        if user.role == 'super_admin' or getattr(user, 'is_client_admin', False):
            return True
        if self.approbateur_niveau2_id == user.id:
            return True
        return user.has_permission('can_approve_veille')
    ...
    def peut_rejeter(self, user):
        ...
        if user.role == 'super_admin' or getattr(user, 'is_client_admin', False):
            return True
        if user.id in [self.approbateur_niveau1_id, self.approbateur_niveau2_id]:
            return True
        return user.has_permission('can_approve_veille')
    
    def __repr__(self):
        return f'<VeilleWorkflow {self.veille_id} - {self.etape_actuelle}>'

# -------------------- VEILLE DOCUMENT --------------------
class VeilleDocument(db.Model):
    """Document attaché à une veille"""
    __tablename__ = 'veille_document'
    
    id = db.Column(db.Integer, primary_key=True)
    veille_id = db.Column(db.Integer, db.ForeignKey('veille_reglementaire.id'), nullable=False)
    
    nom_fichier = db.Column(db.String(255), nullable=False)  # Nom unique sur disque
    nom_original = db.Column(db.String(255), nullable=False)  # Nom d'origine
    type_fichier = db.Column(db.String(50))  # Extension
    ...
    def icon_class(self):
        ...
        return icons.get((self.type_fichier or '').lower(), 'fa-file text-secondary')
    
    def __repr__(self):
        return f'<VeilleDocument {self.id}: {self.nom_original}>'



class VeilleAuditTrail(db.Model):
    """Journal d'audit pour toutes les actions sur les veilles"""
    __tablename__ = 'veille_audit_trail'
    ...
    action = db.Column(db.String(50))  # create, update, archive, restore, delete, approve, reject
    ...
    veille = db.relationship('VeilleReglementaire', backref='audit_trails')
    ...
    def __repr__(self):
        return f'<AuditTrail {self.action} veille={self.veille_id}>'

# ============================================
# LISTENERS SQLALCHEMY
# ============================================
# ⚠️ À PLACER TOUT À LA FIN DE models.py
# APRÈS la définition de VeilleReglementaire et VeilleAuditTrail
...
# ============================================
# LISTENER : Audit Trail Veille Réglementaire
# ============================================

@event.listens_for(VeilleReglementaire, 'before_update')
def veille_before_update(mapper, connection, target):
    """
    Capture les modifications avant update pour alimenter VeilleAuditTrail.
    
    ⚠️ Placé à la fin de models.py pour éviter les imports circulaires.
    """
    from sqlalchemy.orm import Session
    
    # Essayer d'obtenir les infos de contexte Flask (sans crasher si absent)
    user_id = None
    ip_address = None
    user_agent = None
    ...
    try:
        ...
        # Pas dans un contexte Flask (ex: script CLI) → on continue sans
        pass
    
    # Créer une session séparée pour lire l'ancien état
    session = Session(bind=connection)
    
    try:
        old_state = session.query(VeilleReglementaire).get(target.id)
        
        if not old_state:
            return  # Nouvel enregistrement, rien à comparer
        
        champs_a_surveiller = [
            'titre', 'statut', 'impact_estime',
            'date_application', 'date_publication',
            'description', 'reference',
            'type_reglementation', 'organisme_emetteur'
        ]
        
        for champ in champs_a_surveiller:
            old_value = getattr(old_state, champ, None)
            new_value = getattr(target, champ, None)
            
            # Ne logger que si la valeur a changé
            if old_value != new_value:
                trail = VeilleAuditTrail(
                    veille_id=target.id,
                    action='update',
                    champ_modifie=champ,
        ...
        session.rollback()
        print(f"⚠️ Erreur audit trail veille {target.id}: {e}")
    finally:
        session.close()


@event.listens_for(VeilleReglementaire, 'after_insert')
def veille_after_insert(mapper, connection, target):
    """Log la création d'une veille"""
    from sqlalchemy.orm import Session
    ...
    try:
        trail = VeilleAuditTrail(
            ...
            nouvelle_valeur=f"Veille créée : {target.titre}",
            utilisateur_id=user_id,
            ip_address=ip_address,
            user_agent=user_agent,
            client_id=target.client_id
        ...
        session.rollback()
        print(f"⚠️ Erreur audit trail create veille {target.id}: {e}")
    finally:
        session.close()
# ============================================
...
class Notification(db.Model):
    ...
    TYPE_ERROR = 'error'
    TYPE_VEILLE_A_VALIDER = 'veille_a_valider'
    TYPE_VEILLE_A_APPROUVER = 'veille_a_approuver'
    TYPE_VEILLE_EN_VIGUEUR = 'veille_en_vigueur'
    TYPE_VEILLE_REJETEE = 'veille_rejetee'
    TYPE_VEILLE_FAVORI_EN_VIGUEUR = 'veille_favori_en_vigueur' 
    # Niveaux d'urgence
    ...
    def get_icon(self):
        ...
        icons = {
            ...
            'veille_a_valider': 'eye',
            'veille_a_approuver': 'stamp',
            'veille_en_vigueur': 'check-double',
            'veille_rejetee': 'times-circle',
            'veille_favori_en_vigueur': 'star', 
        }
    ...
    def get_color(self):
        ...
        colors = {
            ...
            'veille_a_valider': 'warning',
            'veille_a_approuver': 'info',
            'veille_en_vigueur': 'success',
            'veille_rejetee': 'danger',
            'veille_favori_en_vigueur': 'warning',
        }
